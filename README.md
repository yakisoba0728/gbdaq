...
“지비닥 AI 애널리스트”는 외부 모델 호출 없이 [`lib/ai/fakeAnalyst.ts`](lib/ai/fakeAnalyst.ts)의
...
네트워크 호출이 실패해도 클라이언트에서 같은 휴리스틱으로 폴백하므로 데모가 절대 깨지지 않습니다.
...
- **AI 애널리스트는 시세 기반 휴리스틱**으로만 동작하며 외부 모델/API 키가 필요 없습니다.
